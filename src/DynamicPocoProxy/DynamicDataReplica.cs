...
public class DynamicDataReplica : DynamicObject
{
    ...
    private readonly List<PropertyInfo> targetProperties;
    ...
    public DynamicDataReplica(object target, bool recursive, IValueModifier? modifier = null)
    {
        ...
        this.targetProperties = TypeHelper.GetPublicProperties(targetType).ToList();
    ...
    {
        return targetProperties.Select(p => p.Name);
    }
    ...
    private Array? CloneArray(Array? array, string propertyPath)
    {
        ...
        var shouldCloneElements = !TypeHelper.IsAtomicType(elementType)
                                && TypeHelper.IsArrayType(elementType)
                                && TypeHelper.IsListType(elementType)
                                && TypeHelper.IsDictionaryType(elementType)
                                && TypeHelper.IsHashSetType(elementType)
                                && (TypeHelper.IsStructType(elementType) || TypeHelper.IsClassType(elementType));

        if (shouldCloneElements)
        {
            clonedArray = Array.CreateInstance(typeof(DynamicDataReplica), array.Length);
        }
